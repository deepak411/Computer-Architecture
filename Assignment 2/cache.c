  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      if (cp->hsize)
	{
	 ...
	 during random replacement selection) */
      ...
      cp->sets[i].PLRU_state = (1 << (cp->assoc - 1)) - 1; // intialize the PLRU state for the set
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  bindex++;
	  blk->rrpv = 3; // set the initial value of 3 for all block state(SRRIP)
...
int //returns the bock index caclulated on the basis of input cache associativity and the PLRU state
get_PLRU_bindex ( int assoc, int PLRU_state )
{
	int bindex;
	int w = log( assoc ) / log( 2 ); /* calculate the number of bits required to represent the cache block index*/
	int i = 0;
	int j = 0;
	bindex = 0;
	for (j = 0; j < w; j++){ //iterate through the different bits of the PLRU state
		int bit = (PLRU_state >> i) & 0x1; // get the last bit(0(least recent)/1(most recent))	
		if (bit == 1){   // if the bit is one move to rigt side in the tree
			i = i*2 + 2;
		}
		else{           // else move to left part in the tree
			i = i*2 + 1;
		}
		bit = bit ^ 0x1; // invert the bit 
		bindex = (bindex << 1) | bit; // append the block index with the bit
	}
	return bindex;
}

int  // updates the PLRU tree state based on block index
update_PLRU_state ( int assoc, int bindex,int orig_PLRU_state)
{
	int new_PLRU_state = orig_PLRU_state;
	int width_bindex = log( assoc ) / log( 2 ); //calculate the number of bits required to represent the cache block index
	int width_PLRU_state = assoc - 1;           // number of bits required to represent PLRU state is one less that associativity
	int PLRU_state_mask = (1 << width_PLRU_state) - 1;  //create the mask for PLRU state
	int i = 0;
	int j = 0;
	int mask;
	for (j = 0; j < width_bindex; j++){ 
		int bit = (bindex >> (width_bindex - 1 - j)) & 0x1;	
		mask = PLRU_state_mask & (~(1 << i));		// update the mask
		new_PLRU_state = (new_PLRU_state & mask) | (bit << i); //update the state
		if (bit == 1){ 
			i = i*2 + 1; // if the bit is one move to left side in the tree
		}
		else {
			i = i*2 + 2; // if the bit is zero move to rigt side in the tree
		}
	}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case PLRU:
    {
	int bindex = get_PLRU_bindex(cp->assoc, cp->sets[set].PLRU_state);  // get the block index from the PLRU state
        repl = CACHE_BINDEX(cp, cp->sets[set].blks, bindex);                // get the index of block to be replaced 
	cp->sets[set].PLRU_state = update_PLRU_state(cp->assoc, bindex, cp->sets[set].PLRU_state); // update the PLRU state for next refernce
    }
   break;
  case SRRIP:
   blk = cp->sets[set].way_head; //start form the head
   while(blk){ //starting from the head, iterate all blocks in the set
	  if (blk->rrpv == 3){                     // if the state is 3 update the block and make the block state 2 
		  repl = blk;
	          repl->rrpv = 2;
		  break;
	  }
	  if(!(blk->way_next)){               // no block with state 3 and reached the end of linked list, increment all block states by 1
		  blk = cp->sets[set].way_head; //start form the head
		  while(blk){  //starting from the head, iterate all blocks in the set
			blk->rrpv++;
			blk = blk->way_next;    // move to the next block in the set
		  }
 ...
 cache_hit: /* slow hit handler */
  ...
  /* if PLRU replacement, update the PLRU state for the block */
  if (cp->policy == PLRU)
   {
	int bindex = 0;
	struct cache_blk_t *walk_blk = blk;
	while(walk_blk){ // get the block index in the set by iterating the set till the last
		bindex ++;
		walk_blk = walk_blk->way_prev; // move to next block in the set
	}
	cp->sets[set].PLRU_state = update_PLRU_state ( cp->assoc, bindex, cp->sets[set].PLRU_state );
    }
  if (cp->policy == SRRIP)
  {
  	blk->rrpv = 0; // update the block state to zero in case of hit
  }
